...
vec2 coverRevealUv(vec2 uv) {
  float ia = uRevealImageSize.x / max(uRevealImageSize.y, 1.0);
  float pa = iResolution.x / max(iResolution.y, 1.0);
  vec2 s = pa > ia ? vec2(1.0, ia / pa) : vec2(pa / ia, 1.0);
  return (uv - 0.5) * s + 0.5;
}
...
const HalftoneReveal = ({
  ...
  useEffect(() => {
    ...
    const textureReveal = new Texture(gl, { generateMipmaps: false });

    const uniforms = {
      tMap: { value: texture },
      tMapReveal: { value: textureReveal },
    ...
    img.onload = () => {
      ...
      if (!revealSrc) {
        textureReveal.image = img;
        uniforms.uRevealImageSize.value = [img.naturalWidth, img.naturalHeight];
      }
    };
    ...
    if (revealSrc) {
      ...
      imgReveal.src = revealSrc;
      imgReveal.onload = () => {
        textureReveal.image = imgReveal;
        uniforms.uRevealImageSize.value = [imgReveal.naturalWidth, imgReveal.naturalHeight];
      };
    }
